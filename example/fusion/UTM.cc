...
short LatLonToUTMXY(int ReferenceEllipsoid, const double Lat, const double Long,
    ...
    ZoneNumber = short((LongTemp + 180) / 6) + 1;
    ...
    UTMEasting = (double)(k0 * N * (A + (1 - T + C) * A * A * A / 6 + (5 - 18 * T + T * T + 72 * C - 58 * eccPrimeSquared) * A * A * A * A * A / 120)
                          + 500000.0);
...
void UTMXYToLatLon(int ReferenceEllipsoid, const double UTMEasting, const double UTMNorthing, short UTMZone, bool southHemi,
    ...
    double x, y;
    short ZoneNumber = UTMZone;
    //char* ZoneLetter;
    int NorthernHemisphere = !southHemi; //1 for northern hemispher, 0 for southern
