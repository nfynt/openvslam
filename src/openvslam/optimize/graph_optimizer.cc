...
#include "openvslam/optimize/internal/sim3/gnss_measurement_edge.h"
...
void graph_optimizer::optimize(data::keyframe* loop_keyfrm, data::keyframe* curr_keyfrm,
    ...
    // 全keyframeの姿勢（修正前)をSim3に変換して保存しておく	-	Convert all keyframe postures (before correction) to Sim3 and save
    std::vector<g2o::Sim3, Eigen::aligned_allocator<g2o::Sim3>> Sim3s_cw(max_keyfrm_id + 1);
    // 追加したvertexを保存しておく	-	Save the added vertex
    ...
    for (auto keyfrm : all_keyfrms) {
        ...
        // Check if posture is corrected before optimization
        const auto iter = pre_corrected_Sim3s.find(keyfrm);
        if (iter != pre_corrected_Sim3s.end()) {
            // 最適化前に姿勢を修正済みの場合はその姿勢を取り出してvertexにセットする	-
            //If the posture has been corrected before optimization, take out that posture and set it in vertex
            Sim3s_cw.at(id) = iter->second;
            keyfrm_vtx->setEstimate(iter->second);
        }
        else {
            // 姿勢が修正されていない場合はkeyframeの姿勢をSim3に変換してセットする
            // If the posture is not corrected, convert the posture of keyframe to Sim3 and set it
        ...
        // ループの起点になった点はfixしておく	- Fix the starting point of the loop
        if (*keyfrm == *loop_keyfrm) {
            keyfrm_vtx->setFixed(true);
        }

        // vertexをoptimizerにセット	-	set vertex to optimizer
        keyfrm_vtx->setId(id);
    ...
    // どのkeyframe間にedgeが追加されたかを保存しておく	-	Save between which keyframe the edge was added
    std::set<std::pair<unsigned int, unsigned int>> inserted_edge_pairs;

    // constraint edgeを追加する関数	-	Function to add constraint edge
    ...
    // threshold weight以上のloop edgeを追加する	-	Add a loop edge with a threshold weight or more
    for (const auto& loop_connection : loop_connections) {
        ...
        for (auto connected_keyfrm : connected_keyfrms) {
            ...
            // を追加する - For edges other than current vs loop, only those that exceed the weight threshold are added.
            if ((id1 != curr_keyfrm->id_ || id2 != loop_keyfrm->id_)
                && keyfrm->graph_node_->get_weight(connected_keyfrm) < min_weight) {
                continue;
            }

            // 相対姿勢を計算	-	Calculate relative posture
    ...
    // loop connection以外のedgeを追加する	-	Add edges other than loop connection
    for (auto keyfrm : all_keyfrms) {
        // 片方のkeyframeの姿勢を取り出す	-	Take out the posture of one keyframe
        const auto id1 = keyfrm->id_;

        // covisibilitiesに含まれているかをチェックし，必ず修正前の姿勢を使うようにする
        // (正しく相対姿勢を計算するには両者が修正前である必要があるため)
        //Check whether it is included in covisibilities and always use the posture before correction
        // (Because both need to be before correction in order to calculate relative posture correctly)
        ...
        if (parent_node) {
            ...
            // constraint edgeを追加	-	add
            insert_edge(id1, id2, Sim3_21);
        }

        // loop edgeはweightにかかわらず追加する	-	add loop edge regardless of weight
        ...
        // threshold weight以上のcovisibilitiesを追加する	-	Add covisibilities with more than threshold weight
        ...
        for (auto connected_keyfrm : connected_keyfrms) {
            ...
            // parent-childのedgeはすでに追加しているので除外	-	Exclude parent - child edge because it has already been added
            if (*connected_keyfrm == *parent_node
                || keyfrm->graph_node_->has_spanning_child(connected_keyfrm)) {
                continue;
            }
            // loop対応している場合はすでに追加しているので除外	-	If it supports loop, it has already been added, so it is excluded
    ...
    // 5. 姿勢を更新	-	Posture updated

    {
        std::lock_guard<std::mutex> lock(data::map_database::mtx_database_);

        // 点群の位置修正のために，全keyframeの姿勢(修正後)を保存しておく	-	Save the postures of all keyframes (after correction) to correct the position of the point cloud
        ...
        // 点群の位置を修正	-	Correct position of point cloud
        for (auto lm : all_lms) {
